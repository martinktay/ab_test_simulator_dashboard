...
def main():
    ...
    print(f"Total users match: {'Yes' if total_users == expected_total else 'No'}")
    print(f"Group A users match: {'Yes' if group_a_users == expected_group_a else 'No'}")
    print(f"Group B users match: {'Yes' if group_b_users == expected_group_b else 'No'}")
    ...
    print(f"Relative improvement: {((group_b_rate - group_a_rate) / group_a_rate) * 100:.2f}%")
    ...
    print(f"Z-test significant (α=0.05): {'Yes' if p_value_z < 0.05 else 'No'}")
    print(f"Chi-square statistic: {chi2_stat:.4f}")
    print(f"Chi-square p-value: {p_value_chi2:.6f}")
    print(f"Chi-square significant (α=0.05): {'Yes' if p_value_chi2 < 0.05 else 'No'}")
    ...
    device_analysis = df.groupby(['device', 'group'])['converted'].agg(['count', 'sum', 'mean']).round(4)
    device_analysis.columns = ['Users', 'Conversions', 'Conv_Rate']
    print(device_analysis)
    
    # Channel analysis
    print("\nChannel segmentation:")
    channel_analysis = df.groupby(['channel', 'group'])['converted'].agg(['count', 'sum', 'mean']).round(4)
    channel_analysis.columns = ['Users', 'Conversions', 'Conv_Rate']
    print(channel_analysis)
    
    # Region analysis
    print("\nRegion segmentation:")
    region_analysis = df.groupby(['region', 'group'])['converted'].agg(['count', 'sum', 'mean']).round(4)
    ...
    summary_stats.columns = ['Users', 'Conversions', 'Conv_Rate', 'Avg_Session_Duration', 'Std_Session_Duration', 'Avg_Page_Views', 'Std_Page_Views']
    print(summary_stats)
    ...
    df_clean['engagement_score'] = (df_clean['session_duration_sec'] / 60) * df_clean['page_views']
    ...
    print(f"\n📊 KEY INSIGHTS:")
    print(f"- Total users: {total_users:,}")
    print(f"- Group A: {group_a_users:,} users ({group_a_rate*100:.2f}% conversion)")
    print(f"- Group B: {group_b_users:,} users ({group_b_rate*100:.2f}% conversion)")
    print(f"- Improvement: {((group_b_rate - group_a_rate) / group_a_rate) * 100:.2f}%")
